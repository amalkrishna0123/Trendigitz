...
const FlexContent = ({ ifExists, endpoint: { title, heading, text, img, btn, url}}) => {
  return (
    <>
      <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container z-20 ${ifExists? 'flex flex-row-reverse' : 'flex-row' }`}>
        <div className=' max-w-lg lg:max-w-none w-full md:text-center grid items-center lg:justify-items-center'>
            ...
            <p className=' xl:text-sm my-4 md:my-1 text-slate-900'>{text}</p>
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
            <img src={img} alt=""  className={`w-auto object-fill transitions-theme my-2 lg:my-5 ${ifExists? 'h-72 lg:h-64 md:h-60 rotate-6 hover:rotate-12 ' : 'h-72 lg:h-64 md:h-60 -rotate-6 hover:rotate-12' }`}
            />
        </div>
      </div>
    </>
